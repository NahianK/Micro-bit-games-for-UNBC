...
# It is a verbatim copy of quest_engine/computer_app/robot.py.
# agent.py injects the `config` module into sys.modules before importing this,
# so the `import config` below resolves to the agent's runtime namespace.
# Do not add a config.py to this directory.
#
# WHAT THIS IS FOR
...
class Robot(object):
    ...
    def _set_speaker_volume(self):
        """Apply speaker_volume (0-100) via the daemon. Default 100 if unset."""
        level = getattr(config, 'ROBOT_SPEAKER_VOLUME', 100)
        try:
            level = int(level)
        ...
        if level > 100:
            level = 100
        url = 'http://{}:{}/api/volume/set'.format(
            getattr(config, 'ROBOT_HOST', 'localhost'),
            getattr(config, 'ROBOT_PORT', 8000),
        )
        body = json.dumps({'volume': level}).encode()
        ...
        try:
            ...
            print('[robot] speaker volume set to {}'.format(
                data.get('volume', level)))
        except (urllib.error.URLError, ValueError, OSError) as exc:
            print('[robot] speaker volume not set: {}'.format(exc))
    ...
    def speak(self, path, wait=True, stop_event=None):
        ...
        if not self.can_speak() or not os.path.exists(path):
            if self.can_speak() and not os.path.exists(path):
                print('[robot] WAV missing: {}'.format(path))
            return False

        played = self._play_via_daemon(path)
        if not played:
            ...
            try:
                ...
                print('[robot] play_sound failed: {}'.format(exc))
                return False
    ...
    def _daemon_base(self):
        return 'http://{}:{}'.format(
            getattr(config, 'ROBOT_HOST', 'localhost'),
            getattr(config, 'ROBOT_PORT', 8000),
        )

    def _play_via_daemon(self, path):
        """Play a local WAV through the daemon REST API (same path as volume)."""
        path = os.path.abspath(path)
        url = self._daemon_base() + '/api/media/play_sound'
        body = json.dumps({'file': path}).encode()
        req = urllib.request.Request(
            url, data=body,
            headers={'Content-Type': 'application/json'},
            method='POST',
            ...
            with urllib.request.urlopen(req, timeout=8) as resp:
                if resp.status >= 400:
                    raise urllib.error.HTTPError(
                        url, resp.status, 'play failed', resp.headers, None)
            print('[robot] playing {}'.format(os.path.basename(path)))
            return True
        except Exception as exc:
            print('[robot] daemon play failed ({}): {}'.format(
                os.path.basename(path), exc))
            return self._upload_and_play(path)

    def _upload_and_play(self, path):
        """Upload WAV then play by basename — matches quest_engine bench test."""
        ...
        try:
            ...
            print('[robot] cannot read WAV: {}'.format(exc))
            return False
        payload = (
            '--{b}\r\n'
        ...
        ).format(b=boundary, n=filename).encode() + audio + (
            '\r\n--{b}--\r\n'.format(b=boundary).encode()
        )
        try:
            up = urllib.request.Request(
                self._daemon_base() + '/api/media/sounds/upload',
                data=payload,
                headers={
                    'Content-Type':
            ...
            play = urllib.request.Request(
                self._daemon_base() + '/api/media/play_sound',
                data=json.dumps({'file': filename}).encode(),
                headers={'Content-Type': 'application/json'},
                method='POST',
            )
            with urllib.request.urlopen(play, timeout=8) as resp:
                resp.read()
            print('[robot] playing {} (uploaded)'.format(filename))
            return True
        except Exception as exc:
            print('[robot] upload/play failed: {}'.format(exc))
            return False
