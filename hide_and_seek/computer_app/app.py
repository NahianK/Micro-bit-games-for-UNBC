# app.py — Flask + SocketIO server for the Hide and Seek full setup
#
# ── REUSE NOTE ──────────────────────────────────────────────────────────────
# This file is IDENTICAL to hide_and_seek_test/computer_app/app.py.
...
class GameState:
    ...
    Tracks per-hider state:
      ...
      signal_at_relay — smoothed hider signal at bridge (from HIDE packets)
    ...
    def update_hider_signal(self, hider_id: str, rssi: int):
        """Called when a HIDE packet arrives — hider's signal at the bridge."""
        with self._lock:
            h = self._ensure_hider(hider_id)
