# serial_reader.py — Threaded USB serial reader for the Hide and Seek full setup
#
# ── REUSE NOTE ──────────────────────────────────────────────────────────────
# This file is IDENTICAL in logic to hide_and_seek_test/computer_app/serial_reader.py.
# The packet formats are the same — only the bridge hardware differs between
# the test version (relay hider) and the full version (dedicated bridge micro:bit).
# Structure identical to pass_the_ball_test_5_players/computer_app/serial_reader.py.
# Reused unchanged: threading pattern, run(), _smooth(), _emit_status(), get_status().
# Changed:          _parse() handles HIDE/SEEK/TAGGED packet formats.
# ────────────────────────────────────────────────────────────────────────────
#
# Serial line formats (sent by the dedicated Bridge micro:bit):
#
#   "HIDE:H<id>,<rssi>"
#       Hider H<id> broadcast overheard by the bridge.
#       rssi = hider's signal strength at the bridge.
#       → emits 'hider_signal' event.
#
#   "SEEK:H<id>:<seeker_rssi>,<bridge_rssi>"
#       Seeker's self-reported distance to hider H<id>, relayed by bridge.
#       seeker_rssi = seeker's own RSSI reading from H<id> (proximity measure).
#       bridge_rssi = seeker's signal strength at the bridge.
...
class SerialReader(threading.Thread):
    ...
    def _parse(self, line: str):
        try:
            # All lines are "<message>,<bridge_rssi>"
            parts = line.split(",", 1)
            if len(parts) != 2:
                return
            ...
            # ── HIDE:H<id> — hider broadcast seen by bridge ───────────────────
            if message.startswith("HIDE:"):
                hider_id = message[5:]      # e.g. "H1"
                smoothed = self._smooth("HIDE_" + hider_id, bridge_rssi)
