# serial_reader.py - Treasure Hunt TEST
# Reads serial output from the USB-connected treasure (relay) micro:bit.
# Parses treasure hunt radio packets and emits SocketIO events.
# REUSE: identical pattern in treasure_hunt full version (separate bridge micro:bit sends same format).
...
def _parse_line(raw_line, socketio, state):
    ...
    Line formats (from bridge/relay micro:bit):
        TRS:T1,-72          → treasure T1 signal seen at relay
        HUNT:H1:T1:-65,-58  → hunter H1 is -65 dBm from T1, relay sees hunter at -58
    ...
    if payload.startswith('TRS:'):
        # TRS:T1  (relay_rssi = signal of treasure beacon at relay)
        parts = payload.split(':')
        if len(parts) == 2:
            treasure_id = parts[1]
