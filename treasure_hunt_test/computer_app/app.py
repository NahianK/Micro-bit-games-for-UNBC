# app.py - Treasure Hunt TEST
# Flask + SocketIO dashboard for 3-micro:bit test version.
# REUSE: same structure as pass_the_ball and full treasure_hunt version.
...
class GameState:
    ...
    def update_hunter(self, hunter_id, treasure_id, rssi):
        ...
        # Keep track if this treasure is now the nearest
...
# Wire serial_reader events into state updates
def _on_treasure_signal(data):
    state.update_treasure_signal(data['treasure_id'], data['relay_rssi'])
    socketio.emit('treasure_signal', data)


# Monkey-patch socketio.emit so serial_reader can call it directly
