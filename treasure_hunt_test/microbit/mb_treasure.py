# mb_treasure.py - Treasure Hunt TEST version
# Flash onto treasure micro:bits.
# In the 3-micro:bit test, ONE treasure is plugged into USB and acts as the relay.
# REUSE: minus the serial relay section below = full version mb_treasure.py (see treasure_hunt\microbit\mb_treasure.py)
# RADIO_GROUP=42, RADIO_POWER=3 (lower power = better proximity ranging)
...
RADIO_GROUP = 42
RADIO_POWER = 3    # lower power = better proximity range
...
while True:
    ...
    if text:
        # --- SERIAL RELAY (remove this block for full version) ---
        # This section is only active when the micro:bit is USB-connected.
        # Forwards every received packet to the computer as: <text>,<rssi>
        # In the full version a dedicated bridge micro:bit handles this role.
    ...
    if button_b.was_pressed():
        if (last_claim is not None and
            ...
            # Also print to serial so relay (this device) captures it
            print('{},{}'.format(found_msg, 0))
