(function () {
  ...
  function createCard(hid) {
    ...
    card.innerHTML  = `
      ...
      <div class="relay-rssi">Relay signal: <span class="relay-value">—</span> dBm</div>`;
    return {
      card,
  ...
  function markFound(hid) {
    ...
    if (!refs.found) {
      ...
      // Freeze the fill bar at whatever it shows
    }
  }

  // ── time counter ───────────────────────────────────────────────────────────
  // Counts up from first detection until found, then freezes
  ...
  function renderSnapshot(snap) {
    ...
    for (const [hid, data] of Object.entries(h)) {
      const refs = ensureCard(hid);
      // Restore hide_start from server data if available
  ...
  socket.on("rssi_update", (d) => {
    // Seeker proximity to a specific hider
    updateSeekerProximity(d.hider_id, d.seeker_rssi);
  });

  socket.on("hider_signal", (d) => {
    // Hider signal strength at relay (secondary info)
